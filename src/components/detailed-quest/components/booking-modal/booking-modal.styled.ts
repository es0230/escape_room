...
const BlockLayer = styled.div`
  ...
  width: 100vw;
  height: 100vh;
  background-color: ${({ theme }: any) => theme.color.eclipse};
`;
...
const Modal = styled.section`
  ...
  overflow-y: auto;
  overflow-x: hidden;
  background-color: ${({ theme }: any) => theme.color.nero2};
...
const ModalTitle = styled.h2`
  ...
  margin-bottom: 39px;
  padding: 0;
  font-size: ${({ theme }: any) => theme.font.formsHeading};
  line-height: 120%;
  font-weight: 800;
...
const BookingSubmit = styled(Button)`
  ...
  background-color: ${({ theme }: any) => theme.color.pinkSwan};
  &:focus,
  &:hover {
