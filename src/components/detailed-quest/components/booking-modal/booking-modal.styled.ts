...
const BlockLayer = styled.div`
  ...
  width: 100vw;
  height: 100vh;
  background-color: ${({ theme }: Theme) => theme.color.eclipse};
`;
...
const Modal = styled.section`
  ...
  overflow-y: auto;
  overflow-x: hidden;
  background-color: ${({ theme }: Theme) => theme.color.nero2};
...
const ModalTitle = styled.h2`
  ...
  margin-bottom: 39px;
  padding: 0;
  font-size: ${({ theme }: Theme) => theme.font.formsHeading};
  line-height: 120%;
  font-weight: 800;
...
const BookingSubmit = styled(Button)`
  ...
  background-color: ${({ theme }: Theme) => theme.color.pinkSwan};
  &:focus,
  &:hover {
    background-color: ${({ theme }: Theme) => theme.color.gray};
  }
`;
